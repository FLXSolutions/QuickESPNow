...
typedef struct esp_now_peer_info {
    uint8_t peer_addr[ESP_NOW_ETH_ALEN]; /**< ESPNOW peer MAC address that is also the MAC address
                                            of station or softap */
    uint8_t lmk[ESP_NOW_KEY_LEN]; /**< ESPNOW peer local master key that is used to encrypt data */
    uint8_t channel; /**< Wi-Fi channel that peer uses to send/receive ESPNOW data. If the value is
                        0, use the current channel which station or softap is on. Otherwise, it must
                        be set as the channel that station or softap is on. */
    uint8_t ifidx;   /**< Wi-Fi interface that peer uses to send/receive ESPNOW data */
...
typedef struct esp_now_peer_num {
    ...
    int encrypt_num; /**< Number of encrypted ESPNOW peers, maximum value is
 ...
 */
inline esp_err_t esp_now_get_version(uint32_t* /*version*/) { return ESP_OK; }
 ...
 *          - ESP_ERR_ESPNOW_NOT_INIT : ESPNOW is not initialized
 */
inline esp_err_t esp_now_unregister_recv_cb(void) { return ESP_OK; }
 ...
 *          - ESP_ERR_ESPNOW_NOT_INIT : ESPNOW is not initialized
 */
inline esp_err_t esp_now_unregister_send_cb(void) { return ESP_OK; }
 ...
 * @attention 4. The buffer pointed to by data argument does not need to be valid after esp_now_send
 * returns
 *
 ...
 */
inline esp_err_t esp_now_add_peer(const esp_now_peer_info_t* /*peer*/) { return ESP_OK; }
 ...
 *          - ESP_ERR_ESPNOW_NOT_FOUND : peer is not found
 */
inline esp_err_t esp_now_del_peer(const uint8_t* /*peer_addr*/) { return ESP_OK; }
 ...
 *          - ESP_ERR_ESPNOW_FULL : peer list is full
 */
inline esp_err_t esp_now_mod_peer(const esp_now_peer_info_t* /*peer*/) { return ESP_OK; }
 ...
 *          - ESP_ERR_ESPNOW_NOT_FOUND : peer is not found
 */
inline esp_err_t esp_now_get_peer(const uint8_t* /*peer_addr*/, esp_now_peer_info_t* /*peer*/) {
    return ESP_OK;
}

/**
 * @brief     Fetch a peer from peer list. Only return the peer which address is unicast, for the
 * multicast/broadcast address, the function will ignore and try to find the next in the peer list.
 *
 ...
 *          - ESP_ERR_ESPNOW_NOT_FOUND : peer is not found
 */
inline esp_err_t esp_now_fetch_peer(bool /*from_head*/, esp_now_peer_info_t* /*peer*/) {
    return ESP_OK;
}
 ...
 */
inline bool esp_now_is_peer_exist(const uint8_t* /*peer_addr*/) { return true; }
 ...
 *          - ESP_ERR_ESPNOW_ARG : invalid argument
 */
inline esp_err_t esp_now_get_peer_num(esp_now_peer_num_t* /*num*/) { return ESP_OK; }
 ...
 *          - ESP_ERR_ESPNOW_ARG : invalid argument
 */
inline esp_err_t esp_now_set_pmk(const uint8_t* /*pmk*/) { return ESP_OK; }

/**
 * @brief     Set esp_now wake window for sta_disconnected power management
 *
 * @param     window  how much microsecond would the chip keep waked each interval, vary from 0 to
 * 65535
 *
 * @attention 1. Only when ESP_WIFI_STA_DISCONNECTED_PM_ENABLE is enabled, this configuration could
 * work
 * @attention 2. This configuration only work for station mode and disconnected status
 * @attention 3. If more than one module has configured its wake_window, chip would choose the
 * largest one to stay waked
 * @attention 4. If the gap between interval and window is smaller than 5ms, the chip would keep
 * waked all the time
 * @attention 5. If never configured wake_window, the chip would keep waked at disconnected once it
 * uses esp_now
 *
 * @return
 *          - ESP_OK : succeed
 *          - ESP_ERR_ESPNOW_NOT_INIT : ESPNOW is not initialized
 */
inline esp_err_t esp_now_set_wake_window(uint16_t /*window*/) { return ESP_OK; }
